...
public class ServiceManager {
    ...
    public int stopService(Intent targetIntent) {
        ...
        Service service = mServiceMap.get(serviceInfo.name);
        if (service == null) {
            Log.e(TAG, "can not runnning, are you stopped it multi-times?");
            return 0;
        }
        service.onDestroy();
    ...
    public void preLoadServices(File apkFile) throws Exception {
        ...
        for (Map.Entry<ComponentName, ServiceInfo> entry : mServiceInfoMap.entrySet()) {
            Log.e("TAG", "加载到的服务：" + entry.getKey() + " " + entry.getValue());
        }
    }
}
